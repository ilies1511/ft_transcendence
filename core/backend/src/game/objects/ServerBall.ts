...
export class ServerBall extends SharedBall {
		...
		ft_math.intersection_point | undefined
	{
		...
		if (wall.angular_vel !== 0 && wall.next_normal) {
			const ball_end_pos = this.pos.clone().add(this.speed.clone().scale(delta_time));
	
			// signed distance to surface
			const center_diff_end = new ServerVec2(ball_end_pos.x - wall.center.x, ball_end_pos.y - wall.center.y);
			const signed_dist_end = ft_math.dot(center_diff_end, wall.next_normal);
	
			const r = this.radius * Math.sign(signed_dist);
			const d_start = signed_dist - r;
			const d_end = signed_dist_end - r;
	
			if (d_start * d_end < 0) {
				const fraction = d_start / (d_start - d_end);
				const t_sweep = delta_time * fraction;
	
				const sweep_impact_pos = this.pos.clone().add(this.speed.clone().scale(t_sweep));
	
				// currently lerp; todo: maybe slerp?
				const interp_normal = wall.normal.clone().scale(1 - fraction).add(wall.next_normal.clone().scale(fraction));
				interp_normal.unit();
	
				// if the hit point is within wall segment (project onto wall's direct vector)
				const vec_from_center = new ServerVec2(wall.center.x, wall.center.y);
				vec_from_center.sub(sweep_impact_pos);
				const dist_from_center = Math.abs(ft_math.dot(vec_from_center, wall.get_direct()));
				if (dist_from_center <= (wall.length / 2 + this.radius) + ft_math.EPSILON) {
					wall.interp_normal = interp_normal;
					return {p: sweep_impact_pos, time: t_sweep, wall};
				}
			}
		}
		return (undefined);

...
const UsersPage: PageModule = {
	...
	async afterRender(root: HTMLElement) {
		...
		try {
			...
			// Optional: Fetch current user's friends to disable buttons for existing friends
			// const friendsRes = await fetch(`/api/users/${me.id}/friends`);
			...
			if (users && users.length) {
				...
				list.innerHTML = users
					.map((u: any) => {
						...
						// const isFriend = friends.includes(u.id); // Uncomment if fetching friends

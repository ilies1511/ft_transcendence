...
const FriendListPage: PageModule = {
	...
	async afterRender(root) {
		...
		/* data */
		const [reqs, friendsResp] = await Promise.all([
			fetch(`/api/users/${me.id}/requests`).then(r => r.json()),
			fetch(`/api/users/${me.id}/friends`).then(r => r.json())
		])
		const friendIds: number[] = friendsResp.friends ?? []

		/* accepted */
		if (friendIds.length) {
			const users = await Promise.all(friendIds.map(fetchUser))
			$accepted.innerHTML = users.map(u => li(u)).join('')
			$aEmpty.remove()
		} else $aEmpty.textContent = 'No accepted friends'

		/* pending */
		const pendingRows = reqs.filter((r: any) => r.status === 'pending')
		if (pendingRows.length) {
			const users = await Promise.all(pendingRows.map((r: any) => fetchUser(r.requester_id)))
			...
			$pEmpty.remove()
		} else $pEmpty.textContent = 'No pending requests'

		/* rejected */
		const rejectedRows = reqs.filter((r: any) => r.status === 'rejected')
		if (rejectedRows.length) {
			const users = await Promise.all(rejectedRows.map((r: any) => fetchUser(r.requester_id)))
			$rejected.innerHTML = users.map(u => li(u)).join('')
			$rEmpty.remove()
		} else $rEmpty.textContent = 'No rejected requests'
